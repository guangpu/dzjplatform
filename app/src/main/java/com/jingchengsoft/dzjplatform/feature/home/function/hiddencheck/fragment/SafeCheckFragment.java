...
public class SafeCheckFragment extends MyFragment<HiddenCheckActivity> {
    ...
    protected void initListener() {
        ...
        adapter.setOnItemChildClickListener(new OnItemChildClickListener() {
            ...
            public void onItemChildClick(@NonNull BaseQuickAdapter adapter, @NonNull View view, int position) {
                SafeCheckDetailActivity.start(dataList.get(position).getId());
    ...
    private void getListData(String searchValue, int page) {
        HiddenCheckHttpUtils.getCheckList(searchValue, page * 10, 10, "2", new PretreatmentCallback<String>() {
            @Override
            public void onResponse(@NonNull ApiResponse response) {
                if(response.getData() != null) {
                    List<SafeCheck> list= JSON.parseArray(response.getData(), SafeCheck.class);
                    if(page == 0) {
