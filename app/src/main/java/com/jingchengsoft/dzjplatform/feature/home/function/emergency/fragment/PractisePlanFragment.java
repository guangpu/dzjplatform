...
import com.jingchengsoft.dzjplatform.feature.home.function.emergency.adapter.PractisePlanAdapter;
 ...
 * desc   :  演练计划
 */
public class PractisePlanFragment extends MyFragment<HiddenCheckActivity> {

    private List<PractisePlan> dataList = new ArrayList<>();
    private PractisePlanAdapter adapter;
    private int page = 0;
    private List<String> searchItem = new ArrayList<>();
    private String searchValue = "";

    public static PractisePlanFragment newInstance() {
        return new PractisePlanFragment();
    }
    ...
    protected void initAdapter() {
        adapter = new PractisePlanAdapter(dataList);
    ...
    private void getListData(String searchValue, int page) {
        EmergencyHttpUtils.getPractisePlanList(searchValue, page * 10, 10, new PretreatmentCallback<String>() {
            @Override
            public void onResponse(@NonNull ApiResponse response) {
                if(response.getData() != null) {
                    List<PractisePlan> list= JSON.parseArray(response.getData(), PractisePlan.class);
                    if(page == 0) {
